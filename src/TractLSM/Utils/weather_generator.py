...
"""
Simple weather generator to create idealised meteorological forcing.
...
def main(lat, lon, doy, sw_rad_day, tmin, tmax, rain_day, rh, u0):
    ...
    u: array
        diurnal course of the wind speed [m s-1]
    ...
    vpd = WG.estimate_diurnal_vpd(tday, rh)

    u = WG.estimate_diurnal_windspeed(doy, u0)

    return par, tday, precip, vpd, u
...
class WeatherGenerator(object):
    ...
    def estimate_diurnal_vpd(self, tair, rh):

        """
        Calculates the saturation vapour pressure at a specific
        temperature tair as given in Monteith & Unsworth, 1990.
        Then calculates the VPD using the percentage relative humidity.
        ...
        es = 0.61078 * np.exp(17.27 * tair / (tair + 237.3))
        vpd = es * (100. - rh) / 100.  # kPa

        return vpd

    def estimate_diurnal_windspeed(self, doy, u0):

        """
        Randomly generates an increasing / increasing wind speed profile
        around midday.

        Arguments:
        ----------
        u0: float or array
            wind speed at the start of the day [m s-1]
        ...
        # randomly opt for an increase or decrease in u at midday
        increase = np.random.choice([True, False])

        if increase:
            umid = np.random.uniform(low=1.1 * u0, high=5. * u0)
            usunset = np.random.uniform(low=0.1 * umid, high=0.9 * umid)

        else:
            umid = np.random.uniform(low=0.1 * u0, high=0.5 * u0)
            usunset = np.random.uniform(low=1.1 * umid, high=3. * umid)

        for i in np.arange(1, 48+1):

            hour = i / 2.  # convert 30 min data, 0-47 to 0-23.5
            du = np.random.uniform(low=0., high=0.25)

            if i == 1:
                u[i-1] = u0

            elif (hour >= sunrise) and (hour <= 12.):
                if increase:
                    u[i-1] = np.minimum(umid, u[i-2] + du)

                else:
                    u[i-1] = np.maximum(umid, u[i-2] - du)

            elif (hour > 12.) and (hour <= sunset):
                if increase:
                    u[i-1] = np.maximum(usunset, u[i-2] - du)

                else:
                    u[i-1] = np.minimum(usunset, u[i-2] + du)
