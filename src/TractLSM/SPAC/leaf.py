...
def leaf_temperature(p, trans, Tleaf=None, inf_gb=False):
    ...
    # get conductances
    gH, gb, __ = conductances(p, Tleaf=Tleaf, inf_gb=inf_gb)  # mol m-2 s-1
...
def calc_colim_Ci(p, Cs, Tleaf, photo):
    ...
    Ko = np.maximum(cst.zero, Ko)  # we don't want actual zeros in Km div
...
def calc_photosynthesis(p, trans, Ci_s, photo, smooth=True, Tleaf=None,
    ...
    if given_gs:
        Aj = quad(1., Rleaf - J - (Ci + 2. * gamstar) * gsc / (p.Patm *
                  conv.MILI), (J * (Ci - gamstar) - (Ci + 2. * gamstar) * Rleaf)
                  * gsc / (p.Patm * conv.MILI), large_root=False) + Rleaf
